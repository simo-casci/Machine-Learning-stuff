...
def calculate_error(line_parameters, points, y):
    ...
    cross_entropy = -(np.log(p).transpose() * y + np.log(1-p).transpose() * (1-y)) / m
    return cross_entropy

def gradient_descent(line_parameters, points, y, alpha, epochs=500):
    m = points.shape[0]
    for i in range(epochs):
        p = sigmoid(points * line_parameters)
        gradient = alpha * (points.transpose() * (p - y)) / m
        ...
        x2 = -bias / w2 + x1 * (-w1 / w2)
        draw(x1, x2)
        print('iteration: ', i+1, ' -- error: ', calculate_error(line_parameters, all_points, y))
